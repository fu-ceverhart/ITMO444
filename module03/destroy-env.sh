...
INSTANCEIDS=$(aws ec2 describe-instances --output=text --query 'Reservations[*].Instances[*].InstanceId' --filters "Name=instance-state-name,Values=running,pending" "Name=tag:Name,Values=module3-tag")
...
TARGETARN=$(aws elbv2 describe-target-groups --names CDE --query "TargetGroups[*].TargetGroupArn" --output=text)
echo $TARGETARN
...
ELBARN=$(aws elbv2 describe-load-balancers --names CDE --query "LoadBalancers[*].LoadBalancerArn" --output=text)
echo $ELBARN
